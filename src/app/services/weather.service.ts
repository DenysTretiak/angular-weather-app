...
export class WeatherService {
    ...
    getWeatherData(cityName: string): Observable<[WeatherInterface, ForecastInterface]> {
      ...
      if (cachedData) {
       ...
       return forkJoin([
           this.getCurrentWeatherInfo(cityName),
    ...
    private getCurrentWeatherInfo(name: string): Observable<WeatherInterface> {
      const url = `${this.baseUrl}/weather?q=${name}&units=metric&APPID=${this.apiKey}`
      return this.httpClient.get<WeatherInterface>(url);
    }

    private getForecastData(name: string): Observable<ForecastInterface> {
